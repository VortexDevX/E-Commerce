  ...
  return (
    <div className="min-h-screen bg-background">
      <section className="page-shell grid gap-6 pt-8 lg:grid-cols-[minmax(0,1.2fr)_minmax(340px,0.8fr)]">
        <div className="relative overflow-hidden border border-border bg-card p-6 shadow-card md:p-10">
          <div className="absolute right-0 top-0 h-40 w-40 border-b border-l border-border bg-secondary" />
          <div className="relative max-w-3xl">
            <span className="tag-chip tag-chip-primary mb-6 inline-flex">
              New edit live now
            </span>
            <h1 className="display-font max-w-3xl text-5xl font-semibold leading-[0.95] text-foreground md:text-7xl">
          ...
          <div className="relative mt-10 grid gap-3 sm:grid-cols-3">
            {signals.map((item) => (
              <div key={item.label} className="flex items-center gap-3 border border-border bg-secondary px-4 py-3">
        ...
        <aside className="grid gap-4">
          <Link href={{ pathname: "/products", query: { search: "discount" } }} className="group border border-border bg-foreground p-6 text-white shadow-card">
            <TagIcon className="h-7 w-7 text-[#f4c56a]" />
            <h2 className="display-font mt-5 text-4xl font-semibold text-white">Private-sale energy</h2>
            <p className="mt-3 text-sm leading-6 text-white/70">
              Discounted items, live stock, and quick product comparisons.
            </p>
            <span className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-[#f4c56a]">
          ...
          <div className="border border-border bg-card p-6 shadow-card">
            <div className="flex items-center gap-4">
              <div className="grid h-12 w-12 place-items-center border border-border bg-secondary text-primary">
      ...
      <section className="page-shell py-8">
        <div className="mb-5 flex items-end justify-between">
          ...
          <Link href="/products" className="hidden text-sm font-semibold text-primary hover:text-accent sm:inline-flex">
            View all
          </Link>
        </div>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {departments.map((item, index) => (
            <Link key={item.name} href={item.href} className="group border border-border bg-card p-5 shadow-card">
      ...
      <section className="page-shell py-12">
        <div className="grid gap-6 border border-border bg-card p-6 shadow-card md:grid-cols-[1fr_auto] md:items-center md:p-8">
